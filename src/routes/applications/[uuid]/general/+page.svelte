...
<section>
	<h2>General</h2>
	<dl class="overview">
		<dt>UUID</dt><dd><code>{data.uuid}</code></dd>
		<dt>Type</dt><dd>{summary.type}</dd>
		<dt>Status</dt><dd>{summary.status}</dd>
		{#if firstText(record, ['fqdn'])}<dt>Domains</dt><dd>{firstText(record, ['fqdn'])}</dd>{/if}
		{#if firstText(record, ['git_repository'])}<dt>Repository</dt><dd>{firstText(record, ['git_repository'])}</dd>{/if}
		{#if firstText(record, ['git_branch'])}<dt>Branch</dt><dd>{firstText(record, ['git_branch'])}</dd>{/if}
		{#if nested('environment') || firstText(record, ['environment_name'])}<dt>Environment</dt><dd>{nested('environment') || firstText(record, ['environment_name'])}</dd>{/if}
		{#if nested('server') || firstText(record, ['server_name'])}<dt>Server</dt><dd>{nested('server') || firstText(record, ['server_name'])}</dd>{/if}
	</dl>
</section>
...
<details>
	...
	<p class="danger">This cannot be undone. Type <strong>{summary.name || data.uuid}</strong> exactly.</p>
